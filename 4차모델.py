...
# 예측 결과를 CSV 파일에 저장
def save_prediction_to_csv(predicted_price, latest_price, current_time, filename='predictions.csv'):
    # CSV 파일의 필드명 정의
    fieldnames = ['시간', '실제가', '예측가']

    # 파일 잠금 사용하여 CSV 파일을 엽니다
    ...
    with lock:
        ...
        with open(filename, mode='a', newline='') as file:
            ...
            # 실제가 및 예측가 행 작성
            row = {'시간': hour, '실제가': latest_price, '예측가': predicted_price}
            writer.writerow(row)
...
def retrain_model_with_error(inst_id, model_path='trained_lstm_model_with_rl.h5', look_back=60):
    ...
    if data:
        ...
        if os.path.exists('predictions.csv'):
            df_errors = pd.read_csv('predictions.csv', encoding='latin1')  # 인코딩을 지정하여 오류 방지
    ...
    while True:
        ...
        if data:
            ...
            predicted_price = scaler.inverse_transform(
                np.concatenate((predicted_price, np.zeros((predicted_price.shape[0], scaled_data.shape[1] - 1))), axis=1)
            )[:, 0][0]
            ...
            if latest_price is not None:
                ...
                save_prediction_to_csv(predicted_price, latest_price, current_time)
